        ...
        messages=[
            {
                "role": "user",
                "content": [
                    {"type": "text", "text": "Identify food Items and ESTIMATE their weights.Identify high level food items like vegiterian pasta, beef teriyaki (only if they are present in the image) instead of individual items like onions, spinach etc.. Return a list of food items and their weights (in grams or Ounces) as a List of Tuples in Python format. Output must be in a valid JSON format. Identify at max 3 items. JSON format: {food_items : [{'food_name': 'food_name1', 'weight': 'weight_value1'}, {'food_name': 'food_name2', 'weight': 'weight_value2'}]}"},
                    {
                        "type": "image_url",
                        "image_url": {
...
def parseImageOutput(image_output):
    ...
    try:
        ...
        for food_name in food_names:
            food_dict = food_db.search_similar_foods(food_name, n_results=2)
            for key, value in food_dict.items():
                if key not in food_fact:
                    food_fact[key] = value
        
        for key, value in food_fact.items():
            sol.append(f"{key} : {value}")

        return sol
    except:
        return "Vector DB Error"

def reason_image(iamge_path) -> str|List[str]:
    image_output = get_image_output(iamge_path)
    context = parseImageOutput(image_output)
